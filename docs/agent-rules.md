...
automerge 対象 PR のコンフリクト解消依頼を受け取ったエージェントは、プロンプトに記載された対象ブランチを確認し、別のディレクトリまたは別のブランチでペインが開かれている場合は、対象リポジトリの作業ディレクトリへ移動して対象ブランチをチェックアウトする。

差し戻しを受け取ったエージェントは、次の順で対応すること:
...
4. 現 head SHA で `agent-review-passed-sha: <SHA>` コメントを対象 PR に投稿し、レビュー完了マーカーを付け直す

コンフリクト解消の push で head SHA が変わるため、解消前のレビュー完了コメントは無効になる。**レビュー完了マーカーを現 head SHA で付け直すまで、自動マージは再開しない。**
