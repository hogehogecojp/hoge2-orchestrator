...
- **終了コード 1 で終わった**（引数エラーなど、配布を開始できなかった）: 出力（標準出力・標準エラー）を添えてユーザーにエスカレーションする。失敗を握りつぶして手動でファイルをコピーするなどの代替手段は取らない。
- **終了コード 2 で終わった**（部分成功）: 配布は最後まで実行され、一部だけ書き込みを見送った状態。再実行しても同じ結果になるためエスカレーションはせず、スクリプトが末尾に出すスキップ一覧（`⚠ 配布をスキップした項目が N 件あります:` に続く1件1行）から、スキップした対象と理由をユーザーに報告する。ただし、スキップ対象に `agents/`・`vk-agents-personas/` のファイルが含まれる場合は、**そのメンバー定義・人格ファイルが更新されていない**ことを明記したうえで、後続の作業に進んでよいかを確認する。手動でファイルをコピーするなどの代替手段を取らないのは終了コード 1 と同じ。
