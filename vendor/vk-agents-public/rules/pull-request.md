...
**注記: 非クローズ参照に留めるだけでは、親 issue のクローズは防げません**

1 本の PR では閉じられない親 issue を、サブ issue に分けずに「クローズキーワードを付けず完全 URL だけで参照する」運用でしのぐのは不十分です。PR 本文の書き方は GitHub の自動クローズを止めるだけで、**他の仕組みが issue を閉じるのは止められません**。

- 実例: 1 本目の PR にクローズキーワードを付けていなかったにもかかわらず、自動オーケストレーターがマージを検知したときのバックストップ close（取りこぼし防止のために issue を閉じる処理）が親 issue を閉じ、メタ issue も完了扱いになった。残り 2 本ぶんの未実装スコープが追えなくなり、手動で reopen する必要が生じた。

そのため、必要な変更が単一の PR に収まらないと分かった時点で、**親 issue をサブ issue に分割し、各 PR は自分のサブ issue を `Closes` する** のが正しい対処です。分割の判定と手順は `/vk-kore` の 4-1b（`skills/vk-kore/SKILL.md`）に従ってください。
