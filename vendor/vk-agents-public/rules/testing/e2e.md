...
e2e スイートは全件実行で数十分かかることが常態のため、「誰がその完了を待つか」を決めてから走らせる。**Claude サブエージェントは自分が起動したバックグラウンド処理の完了通知を受け取れず、待たせると永久に停止する。** 原則と取りうる方式は [agent-launch.md](../agent-launch.md)「サブエージェントに長時間処理の完了を待たせない」を正とする（理由はそちらに書いてあるためここでは繰り返さない）。

e2e での具体的な適用:

- **麗美（`vk-ui-tester`）は Codex エンジンでの起動を推奨する。** `codex exec` はフォアグラウンドの同期実行で、スイートの完了まで制御が戻らないため停止しない（エンジンの決まり方自体は [agent-launch.md](../agent-launch.md)「起動エンジンの解決」の優先順位に従う）。
- `claude` エンジンで麗美を起動した場合は、**スイートを起動した時点で「起動した。完了待ち」と司へ返して一度終了する**。このとき、司が完了を判定できる具体物（実行ログ・結果ファイルの絶対パス、完了判定に使えるコマンド）を報告に必ず添える。司はそれをポーリングして完了を検知し、`SendMessage` で麗美を再開させて結果の判定・報告を続けさせる（または司がスイートの実行と待機ごと引き取り、結果を麗美へ渡す）。中間報告はテスト結果の受領ではないため、司はこの時点で合否を判定しない。
- **司がスイートの実行・完了待ちを引き取る場合**は、`Monitor` ツールを使わず、`Bash` を `run_in_background: true` で起動して完了条件を確認する until ループで待つ（理由と雛形は [agent-launch.md](../agent-launch.md)「長時間処理の完了はメインが `Bash` の `run_in_background` で待つ」を正とする）。
- 実行時間が読めない処理（テストデータ投入込みの初回実行、`npm install` からの環境構築など）も、長時間処理と同じ扱いにする。
