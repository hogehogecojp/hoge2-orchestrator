...
未充足（必須の ❌）が残っていれば、その項目に戻って埋め直す。**全 required が ✅ になったら完了**。

**起動コマンドは自分で決めず、いま実行した doctor のレポート末尾（`✅ 必須項目はすべて充足しています` の後ろ）に出ている行をそのまま案内する。** 別マシンの VK Terminals API を使う構成では `up` ではなく `start` が出るなど、判定はレポート側で分岐している。ここに固定の案内を書くと、この構成の利用者へ真逆の案内（手元で使わない GUI を起動する `up`）を出すことになる。分岐が増えてもスキル側を追随させないため、判断は doctor に一任する。

`up` を案内された場合、`up` は起動時に同じ doctor を再実行し、全 required 充足を確認すると A に `setup.completedAt` を記録して次回以降の未セットアップ案内を省く（真実はあくまで毎回の doctor）。
