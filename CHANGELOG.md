...
- [ 不具合修正 ] `vk-orchestrator doctor` が文字列以外の値（`[]` `{}` など）や制御文字だけの値も「設定済み」と数えるため、必須項目がすべて充足と表示された直後にタスクが 1 件も処理されない不具合を修正（使えない値は未充足として数えるよう変更。該当する設定の環境では充足数が変わり、`doctor` を CI に組み込んでいる場合はこれまで通っていた確認が失敗します） (#261)
